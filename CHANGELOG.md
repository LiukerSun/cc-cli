...
### 变更
- **顶层 bypass 快捷入口** - 现在可以直接使用 `ccc -y`，等价于 `ccc run -y`，无需再额外输入 `run`

## [2.5.2] - 2026-03-28

### 修复
- **raw mode 换行兼容修复** - 选择器在 raw mode 下统一使用 `\r\n` 输出，修复部分终端中每一行持续右移导致的错位问题

## [2.5.1] - 2026-03-28

### 修复
- **模型选择器终端渲染修复** - 移除整屏重绘方案，改为静态列表加底部状态行更新，避免部分终端中出现内容错位和乱码

## [2.5.0] - 2026-03-28

### 新增
- **默认启动模型选择器** - 直接执行 `ccc` 时会进入模型选择并运行，不再必须输入 `ccc run`
- **方向键选择模型** - 在终端中可使用 `Up / Down` 选择模型，按 `Enter` 直接运行，减少手动输入编号

### 变更
- **交互入口前移** - `ccc` 更接近一个随手切模型的启动器，而不是先记命令再切换模型的工具
- **兼容原有命令** - `ccc run` 继续保留，非交互环境下仍沿用原有回退逻辑
- **测试同步** - 新增默认启动与方向键选择的单测，并保持现有 smoke test 通过
...
### 新增
- **运行时模型选择器** - `ccc run` 在终端中无参数执行时，会直接列出已配置模型并提示选择，不再强依赖预先切换 current profile

### 变更
- **随手切模型更直接** - 从 `run` 入口就能临时切换到任意已配置模型，更符合 `ccc` 作为多模型快速切换工具的使用目标
- **当前模型自动跟随选择** - 通过交互选择运行的模型会同步更新为新的 current profile，方便下一次继续使用
- **测试覆盖补齐** - 新增 `run` 交互选择的单测，并保持现有 dry-run 和 smoke test 行为稳定
